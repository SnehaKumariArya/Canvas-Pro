...
* ** L-Select Free-Transform Box Tool (`⛶`):** Real-time raycast collision matrix lets you click, drag, shift, and dynamically scale any shape, sticker, or text bounding layer.
...
* **Dynamic RefDes Annotation:** Dedicated attribute field to bind active design text descriptions (e.g., `R1 - 10kΩ`, `U1 - 74LS08`) right beneath the component boundaries on the fly.
...
* **🧲 Intersection Snap-to-Grid Matrice:** Forces lines, arrows, shapes, and connection nodes to stick perfectly to the closest junction point, avoiding shaky routing lines.
* **Live Telemetry Measurement HUD:** A floating dynamic HUD overlay tracks your cursor actions to output precise distance layouts (`L: px`) and rotation tracking (`A: °`) in real time.
